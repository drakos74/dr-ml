...
func main() {
	...
	cap := map[string]recurrent.Capture{
		...
		train: &recurrent.OutputCapture{Network: NewLSTMValueNetwork(bufferSize, layerSize)},
		//buffer: &recurrent.OnceCapture{recurrent.OutputCapture{Network: NewLSTMValueNetwork(bufferSize, layerSize)}},
		//dummy:  &recurrent.DummyCapture{recurrent.OutputCapture{Network: NewLSTMValueNetwork(10, 10)}},
		//evolve: &recurrent.EvolutionCapture{recurrent.OutputCapture{Network: NewLSTMValueNetwork(10, 10)}},
		soft: &recurrent.SoftCapture{Network: NewLSTMProbabilityNetwork(bufferSize, layerSize, 2)},
	}

	// this will capture almost one full cycle for 25 events
	recurrent.Train(2500, recurrent.X(0.03), recurrent.Sine, data, cap)
...
func NewLSTMValueNetwork(bufferSize, hiddenLayerSize int) net.NN {
	...
	builder := rc.NewNeuronBuilder(1, 1, int(hiddenLayerSize)).
		...
		WithActivation(ml.Sigmoid, ml.TanH, ml.Sigmoid)
	return rc.New(bufferSize, lstm.New(*builder), net.NewClip(50, 50))
}

func NewLSTMProbabilityNetwork(bufferSize, hiddenLayerSize, outputSize int) net.NN {
	hls := float64(hiddenLayerSize)
	builder := rc.NewNeuronBuilder(1, 2, int(hiddenLayerSize)).
		...
		SoftMax(outputSize)
	return rc.New(bufferSize, lstm.New(*builder), net.NewClip(50, 50)).OutputTransform(func(matrix xmath.Matrix) xmath.Matrix {
